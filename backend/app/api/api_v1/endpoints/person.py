...
@router.get("/check")
async def health():
    return {"message": "Person Router Here!"}
...
@router.get(
    "/{id}", response_description="Person data retrieved", response_model=Person
)
async def get_person_data(id: PydanticObjectId):
    ...
    if person:
        return person
    raise HTTPException(
        status_code=404, detail="Person with id {0} not found".format(id)
    )


@router.post(
    "/",
    response_description="Person data added into the database",
